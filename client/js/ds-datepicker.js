/*
  Date picker adapted from Digital Scotland Design System

  npm: @scottish-government/pattern-library
...
class DSDatePicker {
  constructor(el, options = {}) {
    ...
    if (this.inputElement.dataset.lang) {
    ...
    if (this.lang === 'cy'){
      ...
      this.monthLabels = ['Ionawr', 'Chwefror', 'Mawrth', 'Ebrill', 'Mai', 'Mehefin', 'Gorffennaf', 'Awst', 'Medi', 'Hydref', 'Tachwedd', 'Rhagfyr'];
      this.guidanceText = 'Gallwch ddefnyddio\'r saethau i ddewis dyddiad';
      this.chooseDate = 'Dewiswch ddyddiad';
      this.selectedDate = 'Y dyddiad a ddewiswyd yw';
      this.selectLabel = 'Dewis';
      this.cancelLabel = 'Canslo';
      this.hiddenLabels = {
        'previousYear': 'Y flwyddyn flaenorol',
        'previousMonth': 'Y mis blaenorol',
        'nextMonth': 'Y mis nesaf',
        'nextYear': 'Y flwyddyn nesaf'
      };
    } else {
  ...
  //<svg class="ds_icon" aria-hidden="false" role="img" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><image xlink:href="${this.imagePath}icons.stack.svg#calendar_today"></image></svg>
  ...
  dialogTemplate(titleId) {
    return `<div class="ds_datepicker__dialog__header ">
      ...
      <caption class="ds_datepicker__dialog__table-caption">${this.guidanceText}</caption>
      <thead>
          <tr>
          <th scope="col" abbr="${this.dayLabels[0]}">${this.dayLabelsAbbr[0]}</th>
          <th scope="col" abbr="${this.dayLabels[1]}">${this.dayLabelsAbbr[1]}</th>
          <th scope="col" abbr="${this.dayLabels[2]}">${this.dayLabelsAbbr[2]}</th>
          <th scope="col" abbr="${this.dayLabels[3]}">${this.dayLabelsAbbr[3]}</th>
          <th scope="col" abbr="${this.dayLabels[4]}">${this.dayLabelsAbbr[4]}</th>
          <th scope="col" abbr="${this.dayLabels[5]}">${this.dayLabelsAbbr[5]}</th>
          <th scope="col" abbr="${this.dayLabels[6]}">${this.dayLabelsAbbr[6]}</th>
          </tr>
      </thead>
      ...
      <button type="button" class="govuk-body ds_button ds_button--select js-datepicker-ok" value="ok" data-button="button-datepicker-ok">${this.selectLabel}</button>
      <button type="button" class="govuk-body ds_button ds_button--cancel  js-datepicker-cancel" value="cancel" data-button="button-datepicker-cancel">${this.cancelLabel}</button>
      </div>`;
  }
  ...
  selectDate(date) {
    ...
    //this.monthInputElement.value = date.getMonth() + 1;
    //this.yearInputElement.value = date.getFullYear();
